...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> ConstraintSystem<E, P>
    for OneShotTestAssembly<E, P>
{
    // allocate a variable
    ...
    pub fn is_satisfied(&self, in_a_middle: bool) -> bool {
        ...
        for (_i, (_vars, this_step_coeffs, next_step_coeffs)) in self.input_gates.iter().enumerate()
        {
            for c in this_step_coeffs.as_ref().iter().skip(1) {
                assert!(
                    c.is_zero(),
                    "input gate must contatain only one coefficient"
                );
            }
            for c in next_step_coeffs.as_ref().iter() {
                assert!(
                    c.is_zero(),
                    "input gate must contatain no next step coefficients"
                );
            }
        }
        ...
        for (i, gate_pair) in self.aux_gates.windows(2).enumerate() {
            ...
            let mut tmp = self
                .get_value(this_gate.0.as_ref()[0])
                .expect("must get a variable value");
            tmp.mul_assign(
                &self
                    .get_value(this_gate.0.as_ref()[1])
                    .expect("must get a variable value"),
            );
            ...
            for (&next_var, next_step_coeffs_coeff) in next_gate
                .0
                .as_ref()
                .iter()
                .rev()
