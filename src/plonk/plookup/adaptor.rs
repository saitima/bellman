    ...
    pub fn from_u8(value: u8) -> std::io::Result<Self> {
        let s = match value {
            ...
            _ => {
                use std::io::{Error, ErrorKind};
                let custom_error =
                    Error::new(ErrorKind::Other, "unknown LC transformation variant");
    ...
    pub fn read<R: Read>(mut reader: R) -> std::io::Result<Self> {
        ...
        match prefix {
            ...
            3u8 => {
                ...
                return Ok(TranspilationVariant::MergeLinearCombinations(
                    variant, subhint,
                ));
            }
            4u8 => {
                ...
                return Ok(TranspilationVariant::IntoMultiplicationGate((
                    subhint_a, subhint_b, subhint_c,
                )));
            }
            _ => {}
        }
...
fn evaluate_over_plonk_variables<
    E: Engine,
    P: PlonkConstraintSystemParams<E>,
    CS: PlonkConstraintSystem<E, P>,
>(
...
fn evaluate_over_plonk_variables_and_coeffs<
    E: Engine,
    P: PlonkConstraintSystemParams<E>,
    CS: PlonkConstraintSystem<E, P>,
>(
...
>(
    ...
    assert!(
        P::CAN_ACCESS_NEXT_TRACE_STEP,
        "Transliper only works for proof systems with access to next step"
    );
    ...
    if num_terms <= P::STATE_WIDTH {
        ...
        scratch_space
            .scratch_space_for_vars
            .resize(P::STATE_WIDTH, cs.get_dummy_variable());
        scratch_space
            .scratch_space_for_booleans
            .resize(P::STATE_WIDTH, false);
        scratch_space
            .scratch_space_for_coeffs
            .resize(P::STATE_WIDTH, zero_fr);
        ...
        let mut next_step_var_in_chain = {
            ...
            scratch_space
                .scratch_space_for_coeffs
                .push(free_term_constant); // add constant
        ...
        for _ in 0..(cycles - 1) {
            scratch_space
                .scratch_space_for_vars
                .resize(consume_from_lc, cs.get_dummy_variable());
            scratch_space
                .scratch_space_for_booleans
                .resize(consume_from_lc, false);
            scratch_space
                .scratch_space_for_coeffs
                .resize(consume_from_lc, zero_fr);
        ...
        // final step - we just make a single gate, last one
        {
            scratch_space
                .scratch_space_for_vars
                .resize(P::STATE_WIDTH - 1, cs.get_dummy_variable());
            scratch_space
                .scratch_space_for_booleans
                .resize(P::STATE_WIDTH - 1, false);
            scratch_space
                .scratch_space_for_coeffs
                .resize(P::STATE_WIDTH - 1, zero_fr);
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> Transpiler<E, P> {
    pub fn new() -> Self {
        Self {
            ...
            transpilation_scratch_space: Some(TranspilationScratchSpace::<E>::new(
                P::STATE_WIDTH * 2,
            )),
            hints: vec![],
...
impl<'a, E: Engine, P: PlonkConstraintSystemParams<E>> PlonkConstraintSystem<E, P>
    for &'a mut Transpiler<E, P>
{
    fn alloc<F>(&mut self, value: F) -> Result<PlonkVariable, SynthesisError>
    where
        F: FnOnce() -> Result<E::Fr, SynthesisError>,
...
impl<E: Engine, P: PlonkConstraintSystemParams<E>> crate::ConstraintSystem<E> for Transpiler<E, P> {
        ...
        AR: Into<String>,
    {
        ...
        Ok(crate::Variable::new_unchecked(crate::Index::Aux(
            self.current_plonk_aux_idx,
        )))
    }
        ...
        AR: Into<String>,
    {
        ...
        Ok(crate::Variable::new_unchecked(crate::Index::Input(
            self.current_plonk_input_idx,
        )))
    }
        ...
        LC: FnOnce(crate::LinearCombination<E>) -> crate::LinearCombination<E>,
    {
        ...
        let (a_has_constant, a_constant_term, a_lc_is_empty, a_lc) =
            deduplicate_and_split_linear_term::<E, Self>(
                a(crate::LinearCombination::zero()),
                &mut self.deduplication_scratch,
            );
        let (b_has_constant, b_constant_term, b_lc_is_empty, b_lc) =
            deduplicate_and_split_linear_term::<E, Self>(
                b(crate::LinearCombination::zero()),
                &mut self.deduplication_scratch,
            );
        let (c_has_constant, c_constant_term, c_lc_is_empty, c_lc) =
            deduplicate_and_split_linear_term::<E, Self>(
                c(crate::LinearCombination::zero()),
                &mut self.deduplication_scratch,
            );
        ...
        match (a_is_constant, b_is_constant, c_is_constant) {
            ...
            (false, false, true) => {
                ...
                let (_new_a_var, _, hint_a) =
                    enforce_lc_as_gates(self, a_lc, one_fr, a_constant_term, true, &mut space)
                        .expect("must allocate A LC as gates for constraint like LC * LC = c1");

                let (_new_b_var, _, hint_b) =
                    enforce_lc_as_gates(self, b_lc, one_fr, b_constant_term, true, &mut space)
            ...
            (true, false, false) | (false, true, false) => {
                ...
                if multiplier == zero_fr {
                    ...
                    let (_, _, hint_c) =
                        enforce_lc_as_gates(self, c_lc, one_fr, zero_fr, false, &mut space)
                    ...
                    let hint = TranspilationVariant::MergeLinearCombinations(
                ...
                let final_lc = subtract_lcs_with_dedup_stable::<E, Self>(
                    final_lc,
                    c_lc,
                    &mut self.deduplication_scratch,
                );
            ...
            (true, true, false) => {
                ...
                let (_, _, hint_lc) =
                    enforce_lc_as_gates(self, c_lc, one_fr, free_constant_term, false, &mut space)
            ...
            (false, false, false) => {
                ...
                let (is_quadratic_gate, _coeffs) =
                    is_quadratic_gate::<E, Self>(&a_lc, &b_lc, &c_lc, &mut self.scratch);
                if is_quadratic_gate {
                    let current_lc_number = self.increment_lc_number();
                ...
                let (_new_c_var, _, hint_c) =
                    enforce_lc_as_gates(self, c_lc, one_fr, c_constant_term, true, &mut space)
...
fn is_quadratic_gate<E: Engine, CS: ConstraintSystem<E>>(
    ...
    if c_is_constant {
        ...
        if a_is_linear
            && b_is_linear
            && c_is_linear
            && a_linear_var == b_linear_var
            && b_linear_var == c_linear_var
        {
            is_quadratic = true;
        } else {
...
fn get_constant_term<E: Engine, CS: ConstraintSystem<E>>(
    lc: &LinearCombination<E>,
) -> (bool, E::Fr) {
    let cs_one = CS::one();
...
fn get_first_variable<E: Engine, CS: ConstraintSystem<E>>(
    lc: &LinearCombination<E>,
) -> (bool, Variable) {
    let cs_one = CS::one();
...
fn get_first_variable_with_coeff<E: Engine, CS: ConstraintSystem<E>>(
    lc: &LinearCombination<E>,
) -> (bool, Variable, E::Fr) {
    let cs_one = CS::one();
...
fn num_unique_values<E: Engine, CS: ConstraintSystem<E>>(
    lc: &LinearCombination<E>,
    scratch: &mut HashSet<crate::cs::Variable>,
) -> (bool, usize) {
    let cs_one = CS::one();
...
fn is_linear_term<E: Engine, CS: ConstraintSystem<E>>(
    lc: &LinearCombination<E>,
    scratch: &mut HashSet<crate::cs::Variable>,
) -> (bool, Variable, E::Fr) {
    let cs_one = CS::one();
...
fn deduplicate_stable<E: Engine, CS: ConstraintSystem<E>>(
    ...
    deduped_vec = deduped_vec
        .into_iter()
        .filter(|(_var, coeff)| !coeff.is_zero())
        .collect();
...
fn subtract_lcs_with_dedup_stable<E: Engine, CS: ConstraintSystem<E>>(
    ...
    let mut deduped_vec: Vec<(crate::cs::Variable, E::Fr)> =
        Vec::with_capacity(lc_0.as_ref().len() + lc_1.as_ref().len());
...
pub struct Adaptor<
    ...
    E: Engine,
    P: PlonkConstraintSystemParams<E>,
    CS: PlonkConstraintSystem<E, P> + 'a,
> {
    cs: &'a mut CS,
...
impl<'a, E: Engine, P: PlonkConstraintSystemParams<E>, CS: PlonkConstraintSystem<E, P> + 'a>
    Adaptor<'a, E, P, CS>
{
...
impl<'a, E: Engine, P: PlonkConstraintSystemParams<E>, CS: PlonkConstraintSystem<E, P> + 'a>
    crate::ConstraintSystem<E> for Adaptor<'a, E, P, CS>
{
    type Root = Self;
        ...
        AR: Into<String>,
    {
        ...
        Ok(match var {
            PlonkVariable(PlonkIndex::Aux(index)) => {
                crate::Variable::new_unchecked(crate::Index::Aux(index))
            }
        ...
        AR: Into<String>,
    {
        ...
        Ok(match var {
            PlonkVariable(PlonkIndex::Input(index)) => {
                crate::Variable::new_unchecked(crate::Index::Input(index))
            }
        ...
        LC: FnOnce(crate::LinearCombination<E>) -> crate::LinearCombination<E>,
    {
        ...
        match hint {
            TranspilationVariant::IntoQuadraticGate => {
                ...
                space
                    .scratch_space_for_vars
                    .resize(P::STATE_WIDTH, dummy_var);
            ...
            TranspilationVariant::IntoMultiplicationGate(hints) => {
                ...
                let a_var = match t_a {
                    ...
                    | hint @ LcTransformationVariant::IntoMultipleGates => {
                        ...
                        .expect(
                            "must allocate A variable to transpile A LC for multiplication gate",
                        );
                ...
                let b_var = match t_b {
                    ...
                    | hint @ LcTransformationVariant::IntoMultipleGates => {
                        ...
                        .expect(
                            "must allocate B variable to transpile B LC for multiplication gate",
                        );
                ...
                let (c_is_just_a_constant, c_var) = match t_c {
                    ...
                    | hint @ LcTransformationVariant::IntoMultipleGates => {
                        ...
                        .expect(
                            "must allocate C variable to transpile C LC for multiplication gate",
                        );
                        ...
                        (
                            false,
                            Some(
                                new_c_var.expect("transpiler must create a new variable for LC C"),
                            ),
                        )
                    }
                    LcTransformationVariant::IsSingleVariable => {
            ...
            TranspilationVariant::MergeLinearCombinations(merge_variant, merge_hint) => {
                ...
                let lc_into_rewriting = match merge_variant {
                    MergeLcVariant::MergeACThroughConstantB => {
                        ...
                        subtract_lcs_with_dedup_stable::<E, Self>(
                            final_lc,
                            c_lc,
                            &mut self.deduplication_scratch,
                        )
...
fn convert_variable_back(plonk_variable: PlonkVariable) -> crate::Variable {
    let var = match plonk_variable.get_unchecked() {
        ...
        crate::plonk::cs::variable::Index::Aux(0) => unreachable!(
            "can not convert aux variable labeled as 0 (does not exist in plonk, dummy gate)"
        ),
...
impl<'a, E: Engine, P: PlonkConstraintSystemParams<E>, C: crate::Circuit<E>>
    AdaptorCircuit<'a, E, P, C>
{
...
impl<'a, E: Engine, P: PlonkConstraintSystemParams<E>, C: crate::Circuit<E>> PlonkCircuit<E, P>
    for AdaptorCircuit<'a, E, P, C>
{
    fn synthesize<CS: PlonkConstraintSystem<E, P>>(
        &self,
        cs: &mut CS,
    ) -> Result<(), SynthesisError> {
        ...
        let c = self
            .circuit
            .replace(None)
            .expect("Must replace a circuit out from cell");
...
fn transpile_xor_using_new_adaptor() {
    ...
    let crs_vals =
        Crs::<Bn256, CrsForLagrangeForm>::crs_42(setup.permutation_polynomials[0].size(), &worker);
    ...
    let mut key_writer =
        std::io::BufWriter::with_capacity(1 << 24, std::fs::File::create("./xor_vk.key").unwrap());
